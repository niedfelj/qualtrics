...
RSpec.describe Qualtrics::DivisionResource do
  ...
  describe '#find' do
    it 'returns a division' do
      ...
      expected_record = Qualtrics::DivisionMapping.extract_single(response, :read)
  ...
  describe '#create' do
    it 'returns created division with id' do
      ...
      expect(resource.create(Qualtrics::Division.new).id).to eq '123'
    end
  end
  ...
  describe '#update' do
    it 'sends the udpate request for the division' do
      ...
      resource.update(Qualtrics::Division.new, id: '123')
